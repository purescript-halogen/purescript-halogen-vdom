...
checkAttributeExistsAndIsEqual maybeNamespace attributeName expectedElementValue element = do
  ...
  case elementValue of
    Nothing → do
      EFn.runEffectFn2 Util.warnAny "checkAttributeExistsAndIsEqual -> missing" { element }
      throwException $ error $ "Expected element to have an attribute " <> Util.quote (Util.fullAttributeName maybeNamespace (ElemName attributeName)) <> " eq to " <> Util.quote expectedElementValue <> ", but it is missing"
    Just elementValue' → do
      EFn.runEffectFn2 Util.warnAny "checkAttributeExistsAndIsEqual -> not missing" { elementValue', expectedElementValue, meta: { maybeNamespace, attributeName, expectedElementValue, element } }
      unless (elementValue' == expectedElementValue) (do
        throwException $ error $ "Expected element to have an attribute " <> Util.quote (Util.fullAttributeName maybeNamespace (ElemName attributeName)) <> " eq to " <> Util.quote expectedElementValue <> ", but it was equal to " <> Util.quote elementValue'
        )
...
checkPropExistsAndIsEqual propName expectedPropValue element = do
  ...
  unless (Fn.runFn2 Util.refEq propValue expectedPropValue) (do
    throwException $ error $ "Expected element to have a prop " <> Util.quote propName <> " eq to " <> Util.quote (Util.anyToString expectedPropValue) <> ", but it was equal to " <> Util.quote (Util.anyToString propValue)
    )
...
checkExtraAttributeNamesIsEmpty extraAttributeNames element = do
  EFn.runEffectFn2 Util.warnAny "checkExtraAttributeNamesIsEmpty" { extraAttributeNames, meta: { element } }
